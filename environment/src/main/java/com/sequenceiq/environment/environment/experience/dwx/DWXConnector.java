...
public class DWXConnector implements DWXApi {
    ...
    public DWXConnector(@Value("${experience.scan.dwx.api.port}") String dwxPort,
                        @Value("${experience.scan.dwx.api.address}") String dwxAddress,
                        @Value("${experience.scan.dwx.api.protocol}") String dwxProtocol,
                        Client client) {
    ...
    private Optional<CpInternalEnvironmentResponse> readResponse(WebTarget target, Response response) {
        ...
        if (response.getStatusInfo().getFamily().equals(SUCCESSFUL)) {
            ...
            LOGGER.info("Calling DWX ( on the following path : {} ) was not successful! Status was: {}", uri, status);
        }
